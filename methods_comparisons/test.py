...
def get_fitted_model(X_train, Y_train, optimizer, n_seed = 189, verbose = 1):
    ...
    model = Mlp()
    model.add(4, input= 17, kernel_initializer = 0.003, kernel_regularizer = 1e-4)
    model.add(1, kernel_initializer = 0.003, kernel_regularizer = 1e-4)
...
#############################
restart = 3
optimizer = NCG(beta_method="fr", c1=1e-4, c2=.3, restart=restart, ln_maxiter = ln_maxiter, l_eps = l_eps, norm_g_eps = norm_g_eps, debug = debug)
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, verbose)
h_fr = model.history
...
optimizer = NCG(beta_method="pr+", c1=1e-4, c2=.4, ln_maxiter = ln_maxiter, l_eps = l_eps, norm_g_eps = norm_g_eps, debug = debug)
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, verbose)
h_pr = model.history
...
optimizer = NCG(beta_method="hs+", c1=1e-4, c2=.9, ln_maxiter = ln_maxiter, l_eps = l_eps, norm_g_eps = norm_g_eps, debug = debug)
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, verbose)
h_hs = model.history 
...
#############################
m=3
optimizer = LBFGS(m=m, c1= 1e-4, c2=0.9, ln_maxiter = ln_maxiter, l_eps = l_eps, norm_g_eps = norm_g_eps, debug = False)
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, verbose)
