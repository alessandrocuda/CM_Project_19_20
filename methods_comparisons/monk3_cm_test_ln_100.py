...
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, 0)
h_pr = model.history 
print_history(f="Monk3", opt="NCG PR+", c1="1e-4", c2=.1, r=restart, m="-", history=model.history, opt_history=optimizer.history, t=t)
print()
ls_stat(ls_maxiter, optimizer.history)
print()
#############################
...
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, 0)
h_hs = model.history 
print_history(f="Monk3", opt="NCG HS+", c1="1e-4", c2=.4, r=restart, m="-", history=model.history, opt_history=optimizer.history, t=t)
print()
ls_stat(ls_maxiter, optimizer.history)
print()
#############################
...
#############################
m = 3
optimizer = LBFGS(m=m, c1= 1e-4, c2=0.4, ln_maxiter = ls_maxiter,  l_eps = l_eps, norm_g_eps = norm_g_eps)
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, 0)
h_lbfgs = model.history 
print_history(f="Monk3", opt="L-BFGS", c1="1e-4", c2=.4, r="-", m=m, history=model.history, opt_history=optimizer.history, t=t)
print()
ls_stat(ls_maxiter, optimizer.history)
