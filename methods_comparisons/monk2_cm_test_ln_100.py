...
optimizer = NCG(beta_method="pr+", c1=1e-4, c2=.1, ln_maxiter = ln_maxiter, l_eps = l_eps, norm_g_eps = norm_g_eps)
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, 0)
h_pr = model.history 
p_pr = rate(model.history["loss_mse_reg"]) 
print_history(f="Monk2", opt="NCG PR+", c1="1e-4", c2=.1, r="-", m="-", history=model.history, opt_history=optimizer.history, t=t)
print()
ls_stat(ln_maxiter, optimizer.history)
print()
...
optimizer = NCG(beta_method="hs+", c1=1e-4, c2=.1, ln_maxiter = ln_maxiter, l_eps = l_eps, norm_g_eps = norm_g_eps)
model, t = get_fitted_model(X_train, Y_train, optimizer, seed, 0)
h_hs = model.history 
p_hs = rate(model.history["loss_mse_reg"]) 
print_history(f="Monk2", opt="NCG HS+", c1="1e-4", c2=.1, r="-", m="-", history=model.history, opt_history=optimizer.history, t=t)
print()
ls_stat(ln_maxiter, optimizer.history)
print()
