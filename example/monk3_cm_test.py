...
#############################
restart_fr = 6
optimizer = NCG(beta_method="fr", c1=1e-4, c2=.1, restart=restart_fr, tol = 1e-12)
...
optimizer = LBFGS(m=3, c1= 1e-4, c2=0.4, tol=1e-20)
model = get_fitted_model(X_train, Y_train, optimizer, seed, 1)
...
plt.grid()
plt.yscale('log')
plt.legend(["NCG - fr - r={}".format(restart_fr),'NCG - pr','NCG - hs', 'BFGS'], loc='upper right', fontsize='large')    
plt.show()
